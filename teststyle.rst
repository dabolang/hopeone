...
包括：`激活账户`_、`设置账户metadata消息`_、`设置账户权限`_、`发送BU`_、`发布资产`_、`转移资产`_、`创建合约`_、`通过发送资产触发合约`_、
`通过发送BU触发合约`_、`在区块链上写日志信息`_。
...
通过发送资产触发合约操作为 Promise 类型，接口调用方式为：
...
通过发送BU触发合约操作为 Promise 类型，该操作接口调用方式：
